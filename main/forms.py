...
class WorkUploadForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "pdf_file": forms.FileInput(
                attrs={"class": "form-control", "accept": ".pdf", "id": "pdf-upload"}
            ),
        }
        labels = {
    ...
    def clean_name(self):
        ...
        if len(name) < 10:
            raise forms.ValidationError(
                "Название работы должно содержать минимум 10 символов"
            )
        return name
...
class CollectionForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if len(name) < 5:
            raise forms.ValidationError(
                "Название коллекции должно содержать минимум 5 символов"
            )
        return name
